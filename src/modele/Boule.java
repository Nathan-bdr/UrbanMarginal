...
public class Boule extends Objet implements Global, Runnable {
	...
	/**
	 * Constructeur qui crée le label de la boule
	 * @param jeuServeur pour communiquer avec le serveur
	 */
	...
	public void tireBoule(Joueur attaquant, Collection lesMurs) {
		...
		if (attaquant.getOrientation() == GAUCHE) {
			...
			posX = attaquant.getPosY() + LARGEURPERSO + 1;
		}
	...
	public void run() {
		...
		// préparer la victime
		Joueur victime = null;
		// pas positif ou négatif pour faire avancer la boule
		int lePas;
		if (attaquant.getOrientation() == GAUCHE) {
			lePas = -PAS;
		} else {
		...
		if(victime != null && !victime.estMort()) {
			...
			if (victime.estMort()) {
				...
				attaquant.gainBoules(2);
				// jour l'animation de la mort
				for (int k=1; k<=NBETAPESMORT; k++) {
					victime.affiche(MORT,  k);
					pause(80, 0);
				}
			} else {
				// remettre le joueur dans la position de repos
	...
	/**
	 * fais une pause d'une durée précise
